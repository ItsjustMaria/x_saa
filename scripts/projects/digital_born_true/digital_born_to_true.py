...
'''
   Script for setting the 'digital born' indicator on a single record of type 'record' to 'true' 
   
   USAGE:
   python migr_street_to_concept.py --env --fund_nr_of_csv_file
...
import os
home_repo = "/opt/lampp/htdocs/test/"  # Replace with actual path
generic_path = os.path.join(home_repo, "generic")
print(f"\nDoes {generic_path} exist? {os.path.exists(generic_path)}")
if os.path.exists(generic_path):
    print(f"Contents: {os.listdir(generic_path)}")
...
project_name = 'digital_born_to_true'         
record_uuids = Path(HOME_REPO,'data',project_name, 'source', f'{fonds}.csv')             #### !!!! Location of uuid from memorix
test_amount = 5
max_workers= 8
...
FILE = Namespace(f"{PREFIX}/resources/recordtypes/File#")
...
def process_one_uuid(uuid, result):
    ...
    result['log'].append({'info' : f"STARTING WITH UUID: {uuid}"})
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if response.status_code != 200:
                result['log'].append({'error' : f"Reading failed for {uuid}"})
        ...
        for record in g.subjects(RDF.type, MEMORIX.Record): 
            ...
            digital_born = str(g.value(record, FILE.digitalBorn))
            #location_description = URIRef(f"{PREFIX}/resources/vocabularies/concepts/{vocabulair}")

            # Create backup of current data
            backup_df = pd.DataFrame({'uuid': [uuid], 'digital born value': [digital_born]})
            result['backup_rows'].append(backup_df)

            try:                           
                # Check if false and - if s -o set to true
                if digital_born == False:
                    g.set((record, FILE.digitalBorn, Literal(True)))
                
                    turtle_changed = True

                    result['log'].append({'info' : f'digital born set to \'True\' for UUID: {uuid}'}) 
                else:
                    result['log'].append({'info' : f'Digital born was already set to True. Nothing changed for uuid: {uuid}'})

            except Exception as e:
                    result['log'].append({'error' : f'Was not able to change the digital born boolean for uuid:, {uuid}, with error: {e}'})
                    result['errors'].append({'error' : f'Was not able to change the digital born boolean for uuid:, {uuid}, with error: {e}'})
            try: 
                if turtle_changed:
                    ...
                    if response.status_code == 200:            
                        result['log'].append({'info' : f"Turtle succesfully updated for uuid: {uuid}"})
                        if response.status_code != 200:
                            time.sleep(3)
                            response = api.get_record(uuid)
                            
                            if response.status_code != 200:
                                result['log'].append({'error': f"Updating failed for {uuid}"})
                                result['errors'].append({'errortext': 'Update failed for uuid', 'uuid': uuid, 'error': response.text})
                                result['status'] = 'failed'       

                    else:            
                        result['log'].append({'error' : f"Turtle update failed for row:  {uuid}"})

                    result['log'].append({'info' : f"Turtle is changed in the script. Check loginfo for details on uuid: {uuid}"})
                else:
                    result['log'].append({'info' : f"Turtle has no changes on uuid: {uuid}"})    

            except Exception as e:
                result['log'].append({'error' : f'Updating of turtle failed for uuid: {uuid}, with error: {e}'})
                result['errors'].append({'errortext' : f'Updating of turtle failed for uuid: {uuid}, with error: {e}'})

    except Exception as e:
        result['log'].append({'error' : f"Failure in the script for row: {uuid} with error : {e}"})
        result['errors'].append({'errortext' : f"Failure in the script for row: {uuid} with error : {e}"})
    return result

def main():
    sys.path.append(str(f'{HOME_REPO}/scripts' ))
    result = {
            'log' : [],
            'errors': [],       # local list, merged into global `errors` afterward
            'backup_rows': [],  # local list of predicates_df copies
            'out_rows': [], # local list for out_file
            'attempts' : 0,
            'retry_attempts' : 3,
            'retried' : [],
        }   
        
    result['log'].append({'info' : f'ENVIRONMENT: {env}'}) 
    input('\t\"COLLECT AND ALTER DATA NOW?\": (Y/N)')
   
    try: 
        #Creating dataframe
        record_uuids_df = pd.read_csv(record_uuids, 
        sep=";",             
        dtype={ "uuid": str
           })

        # record_df = pd.DataFrame({'uuid': record_uuids_df['uuid'].unique()})

        record_df = record_uuids_df[['uuid']].drop_duplicates()
        print(type(record_df))

        result['log'].append({'info' : f'UUID DF CREATED WITH : {len(record_df)} rows'})

    except Exception as e: 
        result['log'].append({'info' : f'FAILED CREATING DATAFRAME: {record_df}'})
        result['log'].append({'error' : f'Error while creating dataframe {e}'})    

    kwrgs = {
            'record_df' : record_df,
            'max_workers' : max_workers, 
            'process_one_uuid' : process_one_uuid, 
    ...
    from general import threadpool_executor

    threadpool_executor.execute_threadpool(result, **kwrgs)
    ...
    with ThreadPoolExecutor(max_workers) as executor:
        ...
        futures = {
            ...
            executor.submit(
                process_one_uuid, uuid, result['log']
            ): uuid
        ...
        for future in tqdm(as_completed(futures), total=len(futures)):x
            ...
            try:
                ...
                result['log'].append({'error' : f"Unhandled exception for uuid {uuid}: {e}"})
