...
import re
import shutil
import stat   # CHANGED: needed to clear the Windows read-only attribute
import sys
import tarfile
import time   # CHANGED: needed for the retry-with-backoff sleep
import zipfile
...
# CHANGED: this entire section (detect_archive_type through
# write_extract_logs, i.e. everything up to the "core steps" divider
# below) is new -- none of it exists in your original script. Copy the
# whole block in as-is.
# --------------------------------------------------------------------------
...
def retry_on_lock(func, description, logger, attempts=6, base_delay=1.0):
    """Calls func() and retries with exponential backoff if it raises
    PermissionError or an OSError with winerror 5 (Access Denied) or 32
    (Sharing Violation) -- the two errors Windows raises when a file is
    momentarily locked by another process, most commonly an antivirus or
    DLP agent scanning the file in place (exactly what you're seeing with
    Total Commander pausing before releasing files on the quarantine
    drive). Any other exception is raised immediately without retrying.
    Re-raises the final error if every attempt is exhausted."""
    delay = base_delay
    last_err = None
    for attempt in range(1, attempts + 1):
        try:
            ...
            if not isinstance(e, PermissionError) and winerror not in (5, 32):
                raise
            last_err = e
            if attempt == attempts:
                break
            logger.warning(f"{description} blocked (attempt {attempt}/{attempts}) - likely an antivirus "
                            f"scan still holding the file; retrying in {delay:.1f}s...")
            time.sleep(delay)
            delay *= 2
    raise last_err
...
def clear_readonly(path):
    """Recursively strips the Windows read-only attribute from every file
    under path. Tar archives often carry Unix permission bits (e.g. 444)
    that tarfile maps onto the Windows read-only attribute on extraction --
    harmless until a later step (move/copy/delete) needs to touch the file,
    at which point Windows raises [WinError 5] Access is denied. Called
    after every extraction as a safety net, regardless of archive type or
    filter used."""
    for dirpath, _dirnames, file_names in os.walk(path):
        for fn in file_names:
            ...
            try:
                ...
                pass  # best-effort; a real permission problem will surface later anyway


def extract_one(archive_path, dest_dir, atype, logger, retry_attempts=6, retry_delay=1.0):
    """Extracts a single archive into dest_dir. Raises on failure.

    The whole extraction is wrapped in retry_on_lock: on a quarantine/AV-
    scanned drive, the antivirus may be scanning the source archive itself
    (or holding a lock on it) at the moment extraction starts. If that
    happens the extraction is retried from scratch after a backoff delay --
    tarfile/zipfile extractall overwriting already-written files on a
    ...
    def do_extract():
        if atype == "zip":
            ...
            # compression, so this covers .tar, .tar.gz/.tgz, .tar.bz2/
            # .tbz2, and .tar.xz/.txz alike.
            # filter="data" (PEP 706) sanitizes metadata on extraction: it
            # avoids replicating owner/permission bits from the source
            # system (which on Windows can otherwise leave extracted files
            # marked read-only), and blocks unsafe entries like absolute
            # paths, path traversal, and symlinks pointing outside
            # dest_dir. This is also what silences the DeprecationWarning
            # about Python 3.14's new extraction defaults.
    ...
    retry_on_lock(do_extract, f"Extracting '{os.path.basename(archive_path)}'", logger,
                  attempts=retry_attempts, base_delay=retry_delay)

    # Belt-and-suspenders: regardless of archive type/filter, make sure
    # nothing extracted ends up unmovable later.
    clear_readonly(dest_dir)


def extract_archives(compressed_source, extract_to, logger, skip_existing=True,
                      retry_attempts=6, retry_delay=1.0):
...
def transfer_one(src, dest_dir, dest_name, mode, dry_run, logger, allow_rename_on_collision=True,
                  retry_attempts=6, retry_delay=1.0):  # CHANGED: two new params, default to old behaviour
    ...
    # ===== CHANGED BLOCK START =====
    # Original had a plain:
    #     if mode == "copy": shutil.copy2(src, dest_path)
    #     else: shutil.move(src, dest_path)
    # Replaced with a retrying version: a quarantine/AV-scanned drive can
    # still be holding a lock on a freshly extracted file when we try to
    # move/copy it, which raises PermissionError / WinError 5 or 32. We
    # also clear a lingering read-only attribute (can happen from a tar's
    # original Unix permission bits) before each retry.
    def do_transfer():
        try:
            if mode == "copy":
    ...
    retry_on_lock(do_transfer, f"Transferring '{os.path.basename(src)}'", logger,
                  attempts=retry_attempts, base_delay=retry_delay)
    # ===== CHANGED BLOCK END =====
...
def process(df, source_root, dest_root, mode, dry_run, logger, retry_attempts=6, retry_delay=1.0):  # CHANGED: two new params, default to old behaviour
        ...
        for idx, src in enumerate(candidates):
            try:
                ...
                dest_path, status = transfer_one(src, dest_dir, dest_name, mode, dry_run, logger,
                                                  ...
                                                  retry_attempts=retry_attempts, retry_delay=retry_delay)  # CHANGED: pass through retry settings
...
def main():
    ...
    parser.add_argument("--source", required=True,
                         help="Root folder to search for archive folders (extraction target, if --compressed-source is used)")  # CHANGED: help text mentions extraction target
    ...
    # ===== CHANGED BLOCK START: new CLI flags, none of these existed before =====
    ...
    parser.add_argument("--retry-attempts", type=int, default=6,
                         help="How many times to retry an extraction or file transfer that fails with a "
                              "transient lock/access error, e.g. an antivirus scanning the file in place "
                              "(default: 6)")
    parser.add_argument("--retry-delay", type=float, default=1.0,
                         help="Initial delay in seconds before the first retry; doubles on each subsequent "
                              "attempt (default: 1.0)")
    # ===== CHANGED BLOCK END =====
    ...
    # ===== CHANGED BLOCK START: entire extraction-stage call, new =====
    if args.compressed_source:
        ...
        ext_successes, ext_errors = extract_archives(
            ...
            retry_attempts=args.retry_attempts, retry_delay=args.retry_delay
        )
    ...
    successes, errors = process(df, args.source, args.dest, mode, args.dry_run, logger,
                                 retry_attempts=args.retry_attempts, retry_delay=args.retry_delay)  # CHANGED: pass through retry settings
