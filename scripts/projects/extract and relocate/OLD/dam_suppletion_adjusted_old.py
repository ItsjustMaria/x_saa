...
4. For every row in the Excel file, it looks for the expected bestandsnaam
   inside that archive's combined file index:
     - Exact bestandsnaam match (case-insensitive) is tried first. Each
...
def main():
    ...
    parser.add_argument("--compressed-source", default=None, help="Folder containing ZIP/TAR/7Z archives to extract into --source before organizing " "(omit to skip extraction and treat --source as already-extracted folders)")
    parser.add_argument("--no-skip-extracted", action="store_true", help="Re-extract archives even if their target folder already exists and is non-empty")
