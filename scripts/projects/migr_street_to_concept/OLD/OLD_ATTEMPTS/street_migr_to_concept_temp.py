...
def normalize_street_name(street: str) -> str:
    ...
    for token in tokens:
        normalized = TOKENS.get(token, token) 
    ...
    for index, row in tqdm(df_record_uuids.head(test_amount).iterrows(), total=df_record_uuids.shape[0]):
        ...
        try:
            ...
            for inst in g.objects(None, SAA.isAssociatedWithModernAddress): 
                ...
                predicates_df['normalized_street'] = normalize_street_name(street_text_val)
                concept_str_val = concept_df['streetTextualValue'].iloc[0]
                concept_df['normalized_street'] = normalize_street_name(concept_str_val)
                
                same_street = (Levenshtein.ratio(str(predicates_df['normalized_street']).lower(), str(concept_df['normalized_street']).lower()) > 0.95 or Levenshtein.distance(str(predicates_df['normalized_street']).lower(), str(concept_df['normalized_street']).lower()) <= 1 or str(predicates_df['streetTextualValue']).lower() in str(predicates_df['adamlink']) or str(predicates_df['normalized_street']).lower() in str(predicates_df['adamlink']))         
                print(same_street)
                ...
                # Extract SCALAR values from the (single-row) DataFrame
                concept_uuid_val = concept_df['concept_uuid'].iloc[0]
