...
HOME_REPO = Path("/opt/lampp/htdocs/test/migr_street_to_concept")
...
def setup_logging():
    ...
    logfile = check_file_exist('logs/', 'str_migr_log', log)
...
'''def fuzzy_match(query, candidates, threshold=95):
...
def get_fuzzy_adamlink(street, adamlink_to_streets, threshold=95):
...
def main():
    ...
    try:
        ...
        errors.append({'errortext' : 'Error while looking up adamlink' , 'error' : e})
    ...
    try:
        ...
        log.info(f'Alternative names list build and added to concept_df')
            ...
            errors.append({'errortext' : 'Error while building list of alternative names' , 'error' : e})
    ...
    try: 
       # Create csv out files for backup and error constructed of the lists
