...
def find_same_street(predicates_df, concept_df):
    ...
    return merge_dfs, predicates_df['concept_uuid']

def find_streets_in_adamlink(concept_df, df_external_data):
    ...
    df_external_data['alt_number'] = df_external_data['adamlink'].str.extract(r'(\d+)')
    ...
    def find_alternatives(row, df_external_data):    
        ...
        if pd.notna(row['alt_number']):
            ...
            alternatives = df_external_data[df_external_data['alt_number'] == number_to_match]
            altlabel = alternatives['altlabel'].tolist()
            label = alternatives['label'].tolist()
    ...
    for index, row in tqdm(df_record_uuids.head(test_amount).iterrows(), total=df_record_uuids.shape[0]):
        ...
        try:
            ...
            for inst in g.objects(None, SAA.isAssociatedWithModernAddress): 
                ...
                concept_df = find_streets_in_adamlink(concept_df, df_external_data)
                ...
                merge_dfs, _ = find_same_street(predicates_df, concept_df)
                print('I made it out')
                ...
                #street_in_adamlink = (Levenshtein.ratio(str(predicates_df['normalized_street']).lower(), str(predicates_df['normalized_street']).lower()) > 0.95 or Levenshtein.distance(str(concept_df['normalized_street']).lower(), str(predicates_df['normalized_street'].lower()) <= 1 or str(predicates_df['streetTextualValue']).lower() in str(df_external_data['straat-label-altlabel']) or str(predicates_df['normalized_street']).lower() in str(df_external_data['straat-label-altlabel']))
