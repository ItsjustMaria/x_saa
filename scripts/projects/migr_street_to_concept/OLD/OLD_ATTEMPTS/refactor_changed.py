    ...
    for index, row in tqdm(df_record_uuids.head(test_amount).iterrows(), total=df_record_uuids.shape[0]):
        ...
        try:
            ...
            for inst in g.objects(None, SAA.isAssociatedWithModernAddress): 
                ...
                if pd.isna(house_number_val) or house_number_val == '' or house_number_val == 'None':
                    extracted_num = predicates_df['extracted_number'].iloc[0]
                    if pd.notna(extracted_num) and extracted_num != '':
                ...
                if pd.isna(number_add_val) or number_add_val == '' or number_add_val == 'None':
                    extracted_add = predicates_df['extracted_number_add'].iloc[0]
                    if pd.notna(extracted_add) and extracted_add != '':
            ...
            logging.error(f"FAILED TRANSFORMATION {uuid} error = {e}")
