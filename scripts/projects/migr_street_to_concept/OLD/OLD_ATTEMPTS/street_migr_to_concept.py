...
def find_alternatives(predicates_df, concept_df, df_external_data):

    # Remove altlabel from adammlink and add to column in two dataframes 
    concept_df['altlabel'] = concept_df['adamlink'].str.extract(r'(\d+)') 

    df_external_data['altlabel'] = df_external_data['adamlink'].str.extract(r'(\d+)')
    ...
    def find_alternatives(row, df_external_data):    
        ...
        if pd.notna(row['altlabel']):
            ...
            alternatives = df_external_data[df_external_data['altlabel'] == number_to_match]
             
            return alternatives['adamlink'].tolist()  # Return all matching alternatives
        return []

    # Add list alternative writings to predicates dataframe
    concept_df['alternative_names'] = concept_df.apply(find_alternatives, axis=1, args=[df_external_data])
    ...
    print(merge_dfs)
    #merge_dfs = concept_df.merge(df_external_data[['altlabel', 'concept_uuid', 'adamlink']], on = 'normalized_street', how='left' )

    predicates_df = merge_dfs

    return predicates_df, merge_dfs, predicates_df['concept_uuid']
    ...
    for index, row in tqdm(df_record_uuids.head(test_amount).iterrows(), total=df_record_uuids.shape[0]):
        ...
        try:
            ...
            for inst in g.objects(None, SAA.isAssociatedWithModernAddress): 
                ...
                predicates_df, merge_dfs, _ = find_alternatives(predicates_df, concept_df, df_external_data)
                ...
                same_street = (Levenshtein.ratio(str(concept_df['normalized_street']).lower(), str(row.normalized_street).lower() > 0.95 or Levenshtein.distance(str(concept_df['normalized_street']).lower(), str(row.normalized_street).lower()) <= 1 or str(row.altlabel).lower() in str(concept_df['alternative_names']).lower() or str(row.normalized_street).lower() in str(concept_df['alternative_names'])))         
                #print(same_street)

                #street_in_adamlink = (Levenshtein.ratio(str(predicates_df['normalized_street']).lower(), str(predicates_df['normalized_street']).lower()) > 0.95 or Levenshtein.distance(str(concept_df['normalized_street']).lower(), str(predicates_df['normalized_street'].lower()) <= 1 or str(predicates_df['streetTextualValue']).lower() in str(df_external_data['adamlink']) or str(predicates_df['normalized_street']).lower() in str(df_external_data['adamlink']))
