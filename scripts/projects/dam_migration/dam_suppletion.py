...
def write_duplicates_log(successes, path):
    ...
    dup_rows_sorted = sorted(dup_rows, key=lambda r: (safe_str(r["toegangsnummer"]), safe_str(r["bestandsnaam_expected"]), r["dest_path"]))
...
def write_error_log(errors, path):
    errors_sorted = sorted(errors, key=lambda r: (safe_str(r["toegangsnummer"]), safe_str(r["uuid"]), safe_str(r["bestandsnaam"])))
