...
project_name = 'description_and_subejct_description'                    #### !!!! State project dir name for data collection in file, logs and data folder     
os.makedirs(Path(HOME_REPO,'data',project_name, 'source'), exist_ok=True) 
ext = 'csv'
file = f'record_uuids_{env}.{ext}'
filename = Path(HOME_REPO,'data',project_name, 'source', file)

if os.path.exists(filename):
        os.makedirs(Path(HOME_REPO,'data',project_name, 'source', 'OLD/'), exist_ok=True)
        old_file = Path(HOME_REPO,'data',project_name, 'source', 'OLD/', f'{file}_{current_datetime}.{ext}')
                        ...
                        "queries": [
                            {
                                "type": "FieldQuery",
                                "operator": "equals",
                                "field": "Deed.saa:hasSpecification.saa:specificationType",
                                "value": "https://stadsarchiefamsterdam.memorix.io/resources/vocabularies/concepts/e8a92b13-efea-4b2e-e053-b784100a3466"
                            },
                            {
                                "type": "FieldQuery",
                                "operator": "equals",
                                "field": "Deed.saa:hasSpecification.saa:specificationType",
                                "value": "https://stadsarchiefamsterdam.memorix.io/resources/vocabularies/concepts/e8a92b13-efeb-4b2e-e053-b784100a3466"
                            }
                        ],
                        "type": "OrQuery"
                    },
...
print(f"Done. Wrote {len(uuids)} UUIDs to {filename}")