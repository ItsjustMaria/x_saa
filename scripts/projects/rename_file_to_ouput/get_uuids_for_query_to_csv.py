...
def check_file_exist(folder, project_name, file, file_type='csv'):
    ...
    filename = Path(HOME_REPO,folder,project_name, f'{file}{ext}')
    
    os.makedirs(Path(filename.parent), exist_ok=True)
    
    if file_type == 'csv' and os.path.exists(filename):
        old_file = Path(HOME_REPO, folder, project_name, 'OLD/', f'{file}_{current_datetime}.csv')
        os.makedirs(Path(HOME_REPO,folder,project_name, 'OLD/'), exist_ok=True)
...
record_uuids = check_file_exist('data/', project_name,f"record_uuids_{env}.csv")
                        ...
                        "queries": [
                            {
                                "type": "FieldQuery",
                                "operator": "equals",
                                "field": "File.file:fonds",
                                "value": "https://stadsarchiefamsterdam.memorix.io/resources/records/b554d686-cd67-6385-e053-b784100a1322"
                            }
                        ],
                        "type": "AndQuery"
                    },
